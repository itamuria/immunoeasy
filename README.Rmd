...
{
  write.csv(count_list[[c]], file = paste0(names(count_list)[c],".csv"))
}

htseq_fpkm <- counts2fpkm_htseq (filename = "htseq_counts.csv", mfl_num = c(569))
...
quant_fpkm <- counts2fpkm_quant (filename = "quant_counts.csv", mfl_num = c(569))
...
subread_fpkm <- counts2fpkm_subread (filename = "subread_counts.csv", mfl_num = c(569))
...
cuff_fpkm <- counts2fpkm_cuff (filename = "cufflink_fpkm.csv", previous_clean = TRUE)
